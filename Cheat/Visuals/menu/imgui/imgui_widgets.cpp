...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    if (hovered || pressed)
        window->DrawList->AddRect(bb.Min, bb.Max, ImColor(130, 20, 20), 4);
...
bool ImGui::ButtonEx2(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    if (hovered || pressed) {
        if (it_anim->second.label_alpha < 255.f)
            it_anim->second.label_alpha += 5.f / ImGui::GetIO().Framerate * 160.f;

        if (it_anim->second.closing_alpha < 255.f)
            it_anim->second.closing_alpha += 15.f / ImGui::GetIO().Framerate * 160.f;
    }
    else {
        ...
        if (it_anim->second.closing_alpha > 0.f)
            it_anim->second.closing_alpha -= 10.f / ImGui::GetIO().Framerate * 160.f;
    }

    // Render
    const ImU32 inside_solid_col = ImGui::GetColorU32(ImGuiCol_WindowBg);
    const ImU32 outside_solid_col = ImColor(0, 0, 0);
    const ImU32 inside_hover_col = ImColor(130, 20, 20, (int)it_anim->second.closing_alpha);
    ...
    ImGui::PushStyleColor(ImGuiCol_Text, ImGui::ColorConvertFloat4ToU32(ImColor(255, 255, 255, 255 - (int)it_anim->second.label_alpha)));
    if (g.LogEnabled)
        ImGui::LogSetNextTextDecoration("[", "]");
    ImGui::RenderTextClipped(bb.Min + style.FramePadding, bb.Max - style.FramePadding, label, NULL, &label_size, style.ButtonTextAlign, &bb);
    ImGui::PopStyleColor();

    ImGui::PushStyleColor(ImGuiCol_Text, ImGui::ColorConvertFloat4ToU32(ImColor(255, 255, 255, (int)it_anim->second.label_alpha)));
    if (g.LogEnabled)
        ImGui::LogSetNextTextDecoration("[", "]");
    ImGui::RenderTextClipped(bb.Min + style.FramePadding, bb.Max - style.FramePadding, label, NULL, &label_size, style.ButtonTextAlign, &bb);
    ImGui::PopStyleColor();
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImRect total_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(w - 10, label_size.y + 10));
    ...
    const char* value_buf_end = value_buf + DataTypeFormatString(value_buf, IM_ARRAYSIZE(value_buf), data_type, p_data, format);
    float pos = 0;
    window->DrawList->AddRectFilled(frame_bb.Min, ImVec2(frame_bb.Max), ImColor(15, 15, 15), 1);

    window->DrawList->AddRectFilled(frame_bb.Min, ImVec2(grab_bb.Min.x + 2, frame_bb.Max.y), ImColor(130, 20, 20), 1);
    //window->DrawList->AddRectFilled(ImVec2(grab_bb.Min.x - 6, frame_bb.Max.y - 7), ImVec2(grab_bb.Min.x + 3, frame_bb.Max.y + 2), ImColor(252, 255, 255), 3);
    window->DrawList->AddCircleFilled((grab_bb.Min), 5.0f, ImColor(252 / 255.f, 255 / 255.f, 255 / 255.f), 30);//124, 43, 186
    RenderText(total_bb.Min, label);
    RenderText({ total_bb.Max.x - label_size.x / 2 -40, total_bb.Min.y }, value_buf, value_buf_end);
...
struct combo_element {
    float open_anim, arrow_anim, checkmark_anim;
};
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    window->DrawList->AddRectFilled(clickable.Min, clickable.Max, ImColor(26, 26, 26), 3.0f);

    window->DrawList->AddText(rect.Min, GetColorU32(ImGuiCol_Text), label);

    RenderTextClipped(clickable.Min + ImVec2(8, 6), clickable.Max - ImVec2(24, -45), preview_value, NULL, NULL, ImVec2(0.0f, 0.0f));
...
bool ImGui::SliderFloat5(const char* label, float v[5], float v_min, float v_max, const char* format, ImGuiSliderFlags flags)
{
    return SliderScalarN(label, ImGuiDataType_Float, v, 0.1, &v_min, &v_max, format, flags);
}
...
#include <xstring>
bool ImGui::SliderScalar_(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ImGuiWindow* window = ImGui::GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    ImRect slider_bb = ImRect(frame_bb.Min + ImVec2(4, label_size.y + 3), frame_bb.Max - ImVec2(50, 8));

    int percent = 0;
    ImRect grab_bb;
    const bool value_changed = ImGui::SliderBehavior(slider_bb, id, data_type, p_data, p_min, p_max, format, ImGuiSliderFlags_None, &grab_bb);
    if (value_changed)
    ...
    window->DrawList->AddRectFilled(ImVec2(slider_bb.Min.x, slider_bb.Min.y), ImVec2(slider_bb.Max.x, slider_bb.Max.y), ImColor(36, 52, 74, (int)(g.Style.Alpha * 255)), 6);
    window->DrawList->AddRect(ImVec2(slider_bb.Min.x, slider_bb.Min.y), ImVec2(slider_bb.Max.x, slider_bb.Max.y), ImColor(58, 79, 110, (int)(g.Style.Alpha * 255)), 6);
    window->DrawList->AddRectFilled(slider_bb.Min, ImVec2(slider_bb.Min.x + (g.Style.Alpha * it_old_value->second), slider_bb.Max.y), ImColor(137 / 255.f, 184 / 255.f, 255 / 255.f, ImGui::GetStyle().Alpha), 6, ImDrawFlags_RoundCornersLeft);
    ...
    char value_buf[64];
    /*const char* value_buf_end = value_buf + ImGui::DataTypeFormatString(value_buf, IM_ARRAYSIZE(value_buf), data_type, (p_data), format, (float)percent / 122200.f);*/
    window->DrawList->AddText(ImVec2(slider_bb.Max.x + 30 - ImGui::CalcTextSize(value_buf).x / 2, slider_bb.Min.y - 4), ImColor(205 / 255.f, 205 / 255.f, 205 / 255.f, g.Style.Alpha), value_buf);
...
{
    return SliderScalar_(label, ImGuiDataType_S32, v, &v_min, &v_max, format, flags);
}
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    if (is_multiline)
    {
        ...
        /// const ImColor bg_col = ImColor((held && hovered) ? ImGuiCol_HeaderActive : hovered ? ImColor(130, 20, 20) : ImColor(42, 42, 43));
    ...
    if (!is_multiline)
    {
        ...
        RenderFrame(frame_bb.Min, frame_bb.Max, ImColor(42, 42, 43), true, 4);
        window->DrawList->AddRect(frame_bb.Min, frame_bb.Max, ImColor(45, 45, 45), 4);
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ...
    const bool alpha = (flags & true) == 0;
    const bool hdr = (flags & ImGuiColorEditFlags_HDR) != 0;
    const int components = false ? 4 : 3;

    // Convert to the formats we need
    float f[4] = { col[0], col[1], col[2],  1.0f };
    ...
    if (label != label_display_end && !(flags & ImGuiColorEditFlags_NoLabel))
    {
        ...
        window->DC.CursorPos = ImVec2(pos.x, pos.y + style.FramePadding.y);
    ...
    if (!(flags & ImGuiColorEditFlags_NoSmallPreview))
    {
        ...
        window->DC.CursorPos = ImVec2(pos.x + GetWindowSize().x - style.WindowPadding.x - 10, pos.y + style.FramePadding.y);
        SetCursorPosX(GetWindowSize().x - style.WindowPadding.x - 16);
        if (ColorButton("##ColorButton", col_v4, flags))
        {
            if (!(flags & ImGuiColorEditFlags_NoPicker))
            {
                ...
                OpenPopup("picker");
                SetNextWindowPos(g.LastItemData.Rect.GetBL() + ImVec2(-1, 2));
            }
        }
        if (!(flags & ImGuiColorEditFlags_NoOptions))
            OpenPopupOnItemClick("context");
        ...
        if (BeginPopup("picker", ImGuiWindowFlags_NoMove))
        {
            ...
            ImGuiColorEditFlags picker_flags_to_forward = ImGuiColorEditFlags_DataTypeMask_ | ImGuiColorEditFlags_PickerMask_ | ImGuiColorEditFlags_InputMask_ | ImGuiColorEditFlags_HDR | ImGuiColorEditFlags_NoAlpha ;
            ImGuiColorEditFlags picker_flags = (flags_untouched & picker_flags_to_forward) | ImGuiColorEditFlags_DisplayMask_ | ImGuiColorEditFlags_NoLabel | ImGuiColorEditFlags_NoAlpha;
    ...
    if (value_changed && picker_active_window == NULL)
    {
        ...
        if ((flags & ImGuiColorEditFlags_DisplayHSV) && (flags & ImGuiColorEditFlags_InputRGB))
        {
            ...
            ColorConvertHSVtoRGB(f[0], f[1], f[2], f[0], f[1], f[2]);
            memcpy(g.ColorEditLastColor2, f, sizeof(float) * 3);
        }
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    if (Button("Copy", ImVec2(flags & ImGuiColorEditFlags_NoAlpha ? 94 : 106, 20)))
        show_color[0] = true;
    SameLine(0, 6);
    if (Button("Paste", ImVec2(flags & ImGuiColorEditFlags_NoAlpha ? 94 : 106, 20)))
        show_color[1] = true;
    ...
    if (alpha_here_[1] > 0)
    {
        SetCursorPosX(GetCursorPosX() + 130);
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    if (hovered || selected)
    {
     ...
     //   window->DrawList->AddCircleFilled((window->DrawList, ImVec2(bb.Max.x - 20, bb.Min.y +  9)), 5.0f, ImColor(64 / 255.0f, 114 / 255.0f, 173 / 255.0f), 30);//130, 20, 20
       // RenderFrame(bb.Min, bb.Max, ImColor(130, 20, 20), false, 0.0f);
    }
    RenderNavHighlight(bb, id, 20);

    if (span_all_columns && window->DC.CurrentColumns)
        PopColumnsBackground();
    else if (span_all_columns && g.CurrentTable)
        TablePopBackgroundChannel();
    ImGui::PushStyleColor(ImGuiCol_Text, ImVec4(selected ? ImColor(255, 255, 255) : hovered ? ImColor(200, 200, 200) : ImColor(110, 110, 110)));
